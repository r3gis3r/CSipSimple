...
import com.csipsimple.ui.CStackUpdater;
...
public class SipService extends Service{
	...
	// Map active account id (id for sql settings database) with acc_id (id for pjsip)
	...
	private class ServiceDeviceStateReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(intent.getAction().equals(ACTION_CONNECTIVITY_CHANGED)){
				...
				if( isValidConnectionForOutgoing() || isValidConnectionForOutgoing() ){
					if(!created){
						sipStart();
					}else{
...
//TODO : handle theses receiver filters (-- take from SipDroid project : sipdroid.org)
...
//         intentfilter.addAction(Receiver.ACTION_VPN_CONNECTIVITY);
		...
		//TODO : autodetect version
         try{
        	 ...
        	 System.load(getApplicationContext().getFileStreamPath(SipService.STACK_FILE_NAME).getAbsolutePath());
        	 has_sip_stack = true;
         }catch(UnsatisfiedLinkError e){
        	 e.printStackTrace();
        	 Log.d(THIS_FILE, "We have a problem with the current stack, go to update panel");
			...
			Thread thread = new Thread(){
				...
				public void run() {
					...
					
					{
				        ...
				        //To avoid crash after hangup -- android 1.5 only but even sometimes crash
...
//						
//					}else{
//						Log.w(THIS_FILE, "Modify account "+acc.display_name+" failed !!! ");
